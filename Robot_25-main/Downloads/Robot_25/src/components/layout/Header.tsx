      ...
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          ...
          {isAuthenticated && user && user.role === "customer" && (
            <Link
              to="/menu"
              className={`font-medium transition-colors ${isActive("/menu")
                  ? "text-orange-500 dark:text-orange-400"
                  : "text-gray-700 dark:text-gray-300 hover:text-orange-500 dark:hover:text-orange-400"
                }`}
            >
          ...
          {isAuthenticated && isChef ? (
            <Link
              to="/chef/dashboard"
              className={`font-medium transition-colors ${isActive("/chef/dashboard")
                  ? "text-orange-500 dark:text-orange-400"
                  : "text-gray-700 dark:text-gray-300 hover:text-orange-500 dark:hover:text-orange-400"
                }`}
            >
        ...
        <div className="md:hidden bg-white dark:bg-gray-800 py-4 px-4 shadow-lg animate-fade-in">
          <nav className="flex flex-col space-y-4">
            ...
            {isAuthenticated && user && user.role === "customer" && (
              <Link
                to="/menu"
                className={`px-4 py-2 rounded-md font-medium ${isActive("/menu")
                    ? "bg-orange-100 dark:bg-orange-900 text-orange-500 dark:text-orange-400"
                    : "text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                  }`}
                onClick={() => setIsMenuOpen(false)}
            ...
            {isAuthenticated && isChef ? (
              <Link
                to="/chef/dashboard"
                className={`px-4 py-2 rounded-md font-medium ${isActive("/chef/dashboard")
                    ? "bg-orange-100 dark:bg-orange-900 text-orange-500 dark:text-orange-400"
                    : "text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                  }`}
                onClick={() => setIsMenuOpen(false)}
